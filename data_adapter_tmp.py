...
class RecordDecoder:
    ...
    def record_2_jpg(self, record: np.ndarray):
        ...
        for hand_landmarks, hand_data in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            thinned_display = (thinned_image * 120).astype(np.uint8)
            ...
            ret, thresh = cv2.threshold(combined_image, 25, 255, cv2.THRESH_TOZERO)
...
class DataHandler:
    ...
    def save_to_csv(self):
        file_name = f"{os.getcwd()}/approaches/canny_edges/csvki/32_mix_thresh_bez_tla.csv"
